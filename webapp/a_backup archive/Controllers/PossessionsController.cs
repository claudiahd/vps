...
||||||| .r64
            var possessions = db.Possessions.OrderBy(p => p.IsReturn).ThenByDescending(p => p.PossessionDateTime).Include(p => p.Drivers).Include(p => p.Vehicles).Include(p => p.Users).Include(p => p.Users1).Include(p => p.PossessionsIssues);           
=======
...
>>>>>>> .r70
        ...
        public async Task<ActionResult> ReturnPossession(MyPossession myPossessionReturn)
        {
<<<<<<< .mine
...
||||||| .r64
            ...
            possessions.ReturnDateTime = myPossessionReturn.ReturnDateTime;
            possessions.IsReturn = possessions.Vehicles.IsAvailable = true;
            possessions.Vehicles.LastODOMeterReading = possessions.ODOMeterReturn = myPossessionReturn.ODOMeterReturn;
            possessions.ETagHolderReturn = myPossessionReturn.ETagHolderReturn;
            possessions.FootMattsReturn = myPossessionReturn.FootMattsReturn;
            possessions.CleanlinessReturn = myPossessionReturn.CleanlinessReturn;
            //possessions.EngineServiceDueReturn = myPossessionReturn.EngineServiceDueReturn;
...
>>>>>>> .r70
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> AddNotice(MyPossession myPossessionReturn)
        {
        ...
        public async Task<ActionResult> Create(int? driverId, int? vehicleId)
        {
            ...
            else
            {
                ...
                var driverListFromDB = await db.Drivers.ToListAsync();
